...
local function TouchOfDeathCheck()
	if UnitExists("target") then
		if ni.spell.cd(touchofdeath) == 0
		 ...
		 and (ni.player.hasglyph(123391)
		 or ni.player.powerraw("chi") >= 3) then
			return true;
		end
	end
	return false;
...
local abilities = {
	...
	["resuscitate"] = function()
		if ni.spell.available(resuscitate) then
			if UnitExists("mouseover")
			 and not cache.coc
			 and ni.spell.valid("mouseover", resuscitate, false, true, true) then
	...
	["legacy of the emperor"] = function()
		if ni.spell.available(legacyoftheemperor) then
			if not cache.mounted
			 and not cache.coc
			 and not cache.dog
			 and not cache.eod
			 and not ni.player.buffs("115921||20217||1126||90363") then
	...
	["legacy of the white tiger"] = function()
		if ni.spell.available(legacyofthewhitetiger) then
			if not cache.mounted
			 ...
			 and not cache.dog
			 and not cache.eod
			 and not ni.player.buffs("116781||17007||1459||61316||24604||90309||126373||126309") then
	...
	["expel harm"] = function()
		if ni.spell.available(expelharm) then
			if not cache.mounted
			 and not cache.tod
			 and ni.player.power("energy") >= 40
			 and ni.player.powerraw("chi") <= 3
			 and ni.player.hp() < 80 then
	...
	["detox"] = function()
		if ni.spell.available(detox) then
			if not cache.mounted
			 and ni.healing.candispel("mouseover")
			 and ni.spell.valid("mouseover", detox, false, true, true) then
		...
		if not cache.mounted
		 and not cache.dog then
			...
			if select(2, GetTalentRowSelectionInfo(2)) == 5 then
				...
				if zensphereremaining <= 1
				 and ni.spell.cd(zensphere) == 0 then
					if ni.members[1].hp < playerhp
					 and not ni.unit.debuff(ni.members[1].unit, 137341)
					 and ni.spell.los(ni.members[1].unit) then
			...
			if ni.spell.available(touchofkarma) then
				if playerhp <= 50
				 and ni.spell.valid("target", touchofkarma, false, true) then
			...
			if playerhp < 30 and GetItemCount(5512, false, false) > 0
	...
	["pause"] = function()
		if cache.mounted
		 ...
		 or ni.unit.buff("target", 117961) then
			return true;
		end
	end,
	["energizing brew"] = function()
		if not cache.tod
		 and ni.spell.available(energizingbrew)
		 and (UnitPowerMax("player") - UnitPower("player")) * (1.0 / select(2, GetPowerRegen("player"))) > 5 then
		...
		if not cache.tod
		 and ni.spell.available(tigereyebrew, true) then
			...
			if tebr < 1 
			 ...
			 and ni.unit.buffremaining("target", risingsunkick, "player") > 0
			 and ni.player.powerraw("chi") >= 2
	...
	["touch of death"] = function()
		if ni.spell.available(touchofdeath) 
		 and ni.spell.valid("target", touchofdeath, true, true) then
			if ni.spell.cd(fortifyingbrew) == 0 then
				local targethp = UnitHealth("target");
				local playerhpmax = UnitHealthMax("player");
				if targethp > playerhpmax
				 and targethp <= (playerhpmax * 0.2 + playerhpmax) then
					ni.spell.cast(fortifyingbrew);
	...
	["tiger palm"] = function()
		if ni.spell.available(tigerpalm) then
			if ni.spell.valid("target", tigerpalm, true, true)
			 and not cache.tod then
				local chi = ni.player.powerraw("chi");
				if chi >= 1
				 and cache.tigerpower <= 3 then
				...
				if (combatbreaker > 0 and energy >= 2)
				 or (combatbreaker <= 2 and combatbreaker > 0) then
					ni.spell.cast(tigerpalm);
					return true;
				end					
			end
	...
	["rising sun kick"] = function()
		if ni.spell.available(risingsunkick) then
			if not cache.tod
			 and cache.tigerpower > 1
			 and ni.spell.valid("target", risingsunkick, true, true) then
	...
	["fists of fury"] = function()
		if ni.spell.available(fistsoffury) then
			if ni.spell.cd(risingsunkick) >= 2
			 and IsSpellInRange(jab, "target") == 1
			 and ni.player.powerraw("energy") <= 30
			 and not cache.tod
			 and cache.tigerpower > 4
			 and (UnitPowerMax("player") - UnitPower("player")) * (1.0 / select(2, GetPowerRegen("player"))) > 3
			 and not ni.player.buff(energizingbrew)
	...
	["chi wave"] = function()
		if ni.spell.available(chiwave) then
			if not cache.tod
			 and ni.spell.valid("target", chiwave, true, true) then
	...
	["blackout kick"] = function()
		if ni.spell.available(blackoutkick) then
			if ni.spell.valid("target", blackoutkick, true, true)
			 and not cache.tod
			 and not ni.vars.combat.aoe then
				local breaker = ni.player.buff(116768);
				if ni.player.powerraw("chi") >= 2
				 or breaker then
					if breaker then
	...
	["jab"] = function()
		if not cache.tod
		 and ni.spell.available(jab)
		 and ni.spell.valid("target", jab, true, true)
		 and UnitPower("player") >= 40 then
			local chi = ni.player.powerraw("chi");
			if ni.vars.combat.aoe then
				if chi < 2
				 and ni.unit.debuffremaining("target", risingsunkick, "player") <= 1 then
					ni.spell.cast(jab);
					return true;
				end
				if chi < 1
				 and cache.tigerpower <= 1 then
					ni.spell.cast(jab);
					return true;
				end
			else
				if chi <= 2
				 and (ni.player.hp() >= 80
				 or ni.spell.cd(expelharm) ~= 0) then
					ni.spell.cast(jab);
					return true;
				end
			end
	...
	["spinning crane kick"] = function()
		if ni.spell.available(spinningcranekick) then
			if ni.vars.combat.aoe
			 and not cache.tod
			 and not ni.player.buff(spinningcranekick)
